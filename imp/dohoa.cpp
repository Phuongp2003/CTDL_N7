...
void initQLMBdata(QLMB_data &thisdata)
{
	...
	thisdata.LoaiMB.mode = UpperAlphanumeric;
...
void CreateLoadingScreen()
{
	...
	DrawRectangleRec(Loading.box, {0, 82, 175, 100});
...
bool Popup_showListHK(UIcontroller &control)
{
	...
	if (control.dataTabCB.data->getNode().getTrangThai() == HoanTat || control.dataTabCB.data->getNode().getTrangThai() == HuyChuyen || control.dataTabCB.dataDSVe.position < 0)
		button[1].isActive = false;
	else
		button[1].isActive = true;
...
{
	CreatePopupBackground();
	if (control.dataTabCB.data->getNode().getTrangThai() == HoanTat || control.dataTabCB.data->getNode().getTrangThai() == HuyChuyen)
		DrawTextEx(FontArial, "Mã chuyến bay: ",
				   {StartPos.x + 300,
					CenterDataSetter(100, StartPos.y + 60 + 100,
									 MeasureTextEx(FontArial, "A", 50, 0).y)},
